...
cd "$(W 1)" && echo "# Final README by Agent-1" > readme.md
$MUT commit -m "agent-1: final md" -w agent-1 2>/dev/null && $MUT push 2>/dev/null

cd "$(W 2)" && echo "Final notes by Agent-2" > notes.txt
$MUT commit -m "agent-2: final txt" -w agent-2 2>/dev/null && $MUT push 2>/dev/null
...
EOF
$MUT commit -m "agent-3: final json" -w agent-3 2>/dev/null && $MUT push 2>/dev/null
...
with open('report.docx', 'wb') as f:
    f.write(buf.getvalue())
"
$MUT commit -m "agent-4: final docx" -w agent-4 2>/dev/null && $MUT push 2>/dev/null
...
"
$MUT commit -m "agent-5: final pdf" -w agent-5 2>/dev/null && $MUT push 2>/dev/null
